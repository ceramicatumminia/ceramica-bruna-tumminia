...
export default function Lavorazione() {
  ...
  return (
    <section className={styles.sectionAlt}>
      ...
      <p className={styles.bodySmall} style={{ maxWidth: '620px', marginBottom: '56px' }}>{intro}</p>
      ...
      >
        {fasi.map((f, i) => (
          ...
          >
            <div style={{ fontFamily: 'Playfair Display,serif', fontStyle: 'italic', fontSize: '36px', color: 'var(--bronze-light)', opacity: 0.45, marginBottom: '10px' }}>
              {String(f.ordine).padStart(2, '0')}
            </div>
            <div className={styles.label} style={{ marginBottom: '10px', color: 'var(--terra)' }}>{f.titolo}</div>
            <p className={styles.bodySmall} style={{ fontSize: '15px' }}>{f.descrizione}</p>
          </div>
        ))}
      </div>
